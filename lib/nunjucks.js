...
/**
 * Converts a URL to an absolute URL. Handles protocol-relative
 * CDN URLs and root-relative local paths.
 *
 * @param {string} url
 * @returns {string}
 * @example
 * {{ image.fields.file.url | absoluteUrl }}
 */
env.addFilter('absoluteUrl', function (url) {
  if (!url) return '';
  if (url.startsWith('//')) return `https:${url}`;
  if (url.startsWith('/')) return `${process.env.SITE_URL || ''}${url}`;
  return url;
});
